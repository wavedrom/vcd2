...
const commandHandler = ($, cmd, str) => {
  if ($.commandTail !== '') {
    str = $.commandTail + str; // continuation from previous chunk(s)
  }
  const tailPart = str.slice(-4);
  if (tailPart !== '$end') { // middle of the commnd span
    $.commandTail = str;
    return;
  }
  $.commandTail = '';
  str = str.slice(0, -4).trim(); // remove $end and spaces around
