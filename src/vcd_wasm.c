...
__attribute__((import_module("env"), import_name("onTimeStamp"))) void onTimeStamp(void);
__attribute__((import_module("env"), import_name("onIdString"))) void onIdString(const unsigned char*, const int32_t, const uint8_t, const uint64_t*, const int, const uint64_t*, const int);
...
int onId (vcd_parser_t* state, const unsigned char* _p, const unsigned char* _endp) {
  ...
  *(char *)(p + plen) = 0; // null instead of space
  ...
  if (id <= 0) {
    onIdString(p, id, state->command, value, state->digitCount, mask, state->maskCount);
  }
